...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, input_tensor, T_out, return_all_timesteps = False):
        ...
        print("backbone_output shape in sample",backbone_output.shape)
        ...
        for frag_idx in tqdm(range(T_out // T_in), desc="sampling frags:", disable=(input_tensor.device == 0)):
            ...
            print("slicing shapes are ",frag_idx * T_in, (frag_idx + 1) * T_in)
        ...
        print("final backbone_output shape in sample",backbone_output.shape)
        print("final frames_pred shape in sample",frames_pred.shape)
    ...
    def forward(self, input_tensor, target_tensor, compute_loss=True, **kwargs):
        ...
        if compute_loss:
            frames_in = input_tensor
            frames_gt= target_tensor
